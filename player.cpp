...
bool Player::Go(const vector<string>& args)
{
	...
	if(exit == NULL)
	{
		cout << "\nYou can not go '" << args[1] << "'.\n";
		return false;
	}

	if(exit->locked)
	{
		cout << "\nYou can not access here yet, find a way.\n";
		return false;
	}
...
{
	Creature *target = (Creature*)parent->Find(args[2], CREATURE);

	if (target == NULL)
	{
		cout << "\nCan not find " << args[2] << " in this room.\n";
		return false;
	}
	...
	if (items.size() > 0)
	{
		cout << "\nYou have learned from " << target->name << "'s this:\n";
		...
		cout << "\nYou have already talked with " << target->name << ".\n";
